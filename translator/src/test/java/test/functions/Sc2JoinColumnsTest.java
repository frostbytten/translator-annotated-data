...
public class Sc2JoinColumnsTest {
  private static final Check emptyArgs = new Builder(false).build();

  private static final Check fullArgs =
      new Builder(true).withJoiner("-").addColumn(1).addColumn(2).addColumn(3).build();
  ...
  private static final Check repeatedColumns =
      new Builder(true).withJoiner("-").addColumn(1).addColumn(1).addColumn(2).addColumn(3).build();

  private static final Check noJoiner =
      new Builder(true).addColumn(1).addColumn(2).addColumn(3).build();

  private static final Check onlyJoiner = new Builder(false).withJoiner("-").build();

  private static final List<Arguments> provider =
      List.of(
    ...
    public Check build() {
      ...
      if (this.columns.size() == 0) {
        ...
        return new Check(
            json,
            new CheckFrame(
                this.joiner, this.columns.stream().mapToInt(i -> i).toArray(), this.valid));
      }
    }
  }
}
