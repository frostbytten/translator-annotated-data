...
public class Sc2SheetTest {
    private static final SheetCheck sheetMinimal = new SheetCheckBuilder(true)
            .withContext(MIME_CSV)
            .build();

    private static final SheetCheck sheetComplete = new SheetCheckBuilder(true)
    ...
    private static final SheetCheck sheetXlsxNoName = new SheetCheckBuilder(false)
            .withContext(MIME_XLSX)
            .build();

    private static final SheetCheck sheetDERLessThanDSR = new SheetCheckBuilder(false)
            .withContext(MIME_CSV)
            .withDataStartRow(SDSR_VAL)
            .withDataEndRow(SDER_INVAL)
            .build();

    private static final SheetCheck sheetDSRLessThanDefault = new SheetCheckBuilder(false)
            .withContext(MIME_CSV)
            .withDataStartRow(SD_R_INVAL)
            .build();

    private static final SheetCheck sheetDERLessThanDefault = new SheetCheckBuilder(false)
            .withContext(MIME_CSV)
            .withDataEndRow(SD_R_INVAL)
            .build();

    private static List<Arguments> provider = List.of(
